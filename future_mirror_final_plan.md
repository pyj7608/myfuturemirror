...
5. 응답: `{ message: "반응+질문 통합 메시지", example: "입력 예시" }`
   - `message`:
     - **일반 단계**: 공감 반응 + 다음 질문을 하나의 자연스러운 흐름으로 (3~4문장, 과도한 칭찬 금지)
     - **예외 — Step 4 (`role_details`) 진입 시**: `goal_date` 답변 직후이므로 반응·추임새 없이 바로 질문 시작 (1~2문장, 간결하게)
   - `example`: textarea 단계에서만 제공하는 입력 예시 ("예) "로 시작, 지금까지의 인터뷰 맥락 반영)
